#------------------------------------------------------------------------------#
#
# Instantiate functions utilized by the CANOPY optimization method
#
#------------------------------------------------------------------------------#
...
microbenchmark(Obj(V_ij)) # with 200k i/j combos, median run time is 8ms. That's not too bad.
# /!\ Use Rprof() to see if 
...
microbenchmark(updateV(V_ij, sample(j.u, 1), sample(j.u, 1))) # about 2ms. Not too bad.
...
microbenchmark(GenProposal(r_j, TransMatrix = c2c.inv, lb = vLowerBound, ub = vUpperBound))
  # Run time is about 1.5ms per call. That's good.
